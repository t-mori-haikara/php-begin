...
このリポジトリに含まれる実際のPHPファイルは、本文の最後まで進めた状態に近い内容です。  
本文では学習のために、同じファイルを段階的に書き換えながら進めます。

## まず動かす

このリポジトリをそのまま試す場合は、同梱しているDocker環境を使うのが一番簡単です。  
PHPやApacheを手元に直接インストールしていなくても動かせます。
...
ブラウザで `http://localhost` を開いて、「こんにちは世界！」と表示されればOKです。  
`http://localhost/list` では「何かの一覧画面」と表示されます。
...
ここから先は、本文を上から順に写経していくための説明です。  
新しいディレクトリを作り、少しずつファイルを書き換えながら進めます。

ローカル環境で進める場合は、Apache + PHP8.0以上 + Composerが動く環境を用意します。また、mod_rewriteが有効になっている必要があります。  
Dockerで進める場合も、考え方は同じです。本文中の `composer` コマンドは、必要に応じて `docker compose exec php composer ...` のように読み替えてください。
...
#### クエリパラメータの取り出し
`$request->getQueryParams()`でURLに含まれるクエリパラメータを配列として一括で取得しています。  
...
具体的には`$responseFactory->createResponse()`との記述がそれにあたります。  
引数を省略した場合は、ステータスコード200のレスポンスが作られます。
...
HTTPヘッダーの詳細をいま知る必要はありませんが、`SapiEmitter`が適切に処理してくれていることは覚えておきましょう。

### 確認してみよう

- `http://localhost/?name=太郎` にアクセスして、「こんにちは太郎！」と表示されることを確認する。
- `$response->getBody()->write(...)` の文字列を変えて、Responseに書き込んだ内容がブラウザへ返ることを確認する。

## ルーティング

いまのままだと、どんなURL（たとえばhttp://localhost/product/1）でアクセスされても、  
...
とりあえずブラウザでアクセスしてみてください。正しく設定できていれば、
...
- `http://localhost` と `http://localhost/list` で表示が分かれることを確認する。
- `/hello` というルートを追加して、別の文字列を表示してみる。
...
- `TopAction`に切り出したあとも、`http://localhost/?name=太郎` の表示が変わらないことを確認する。
- 余裕があれば、`/list` の処理も `ListAction` のようなActionクラスへ切り出してみる。
...
ユーザー入力を検証することも大切ですが、「入力として許可するか」と「HTMLとして出力してよい形にするか」は別の話です。  
HTMLに値を埋め込むときは、出力時に`htmlspecialchars`という関数で値をエスケープします。
...
`htmlspecialchars`などという長ったらしい関数を何度も書く苦行から解放されましたね。    
...
`{{ name }}`の部分に、`render`メソッドの第二引数で渡した`name`の値が埋め込まれる仕組みです。  
Twigの独自構文は他にも`if`や`for`など色々あるので調べてみてください。

### 確認してみよう

- `http://localhost/?name=太郎` にアクセスして、Twigテンプレート経由でも「こんにちは太郎！」と表示されることを確認する。
- `http://localhost/?name=<script>alert(1)</script>` のような値を渡して、`<script>`がHTMLとして実行されず文字として表示されることを確認する。

## Dependency InjectionとDIコンテナ

現状のルーティング設定には、実は大きな問題があります。  
ルーティングコールバックとして、Actionクラスをnewでインスタンス化してから渡している点です。  
`new TopAction(...)` のように書いた時点で、そのActionクラスのコンストラクタが実行されます。  
同じようなルート定義が増えると、どのルートにリクエストがあったかにかかわらず、複数のActionクラスが先に生成されてしまいます。  
トップページへのアクセスなのに、無関係なお問い合わせ画面のActionまで作られるのはおかしいですよね。
...
public function __invoke(ServerRequestInterface $request): ResponseInterface
{
    ...
    $html = $twig->render('top.twig.html', [
        'name' => $name,
    ]);
...
これでルート登録時にActionクラスのコンストラクタが実行されてしまうことはなくなりますが、  
...
// ServerRequestInterfaceを要求されたら、現在のHTTPリクエストからRequestを作る
$container->add(ServerRequestInterface::class, function () {
    return ServerRequestFactory::fromGlobals($_SERVER, $_GET, $_POST, $_COOKIE, $_FILES);
});

// Routerを要求されたら、DIコンテナと連携済みのRouterを作る
...
- `$router->get('/', TopAction::class);` のようにクラス名でルート登録しても、トップページが表示されることを確認する。
- `dependencies.php`の`TopAction::class`定義を一時的にコメントアウトすると、依存関係を解決できずエラーになることを確認して、元に戻す。

ここでは、依存関係を理解しやすくするために`TopAction`の引数を手動で定義しています。  
しかし、新しくActionクラスを作るたびに、設定をdependencies.phpに追記するのでしょうか？  
...
`league/container`には`ReflectionContainer`という仕組みがあり、コンストラクタの型宣言を見て依存関係を解決するautowiringもできます。  
ただし、この記事では「どのクラスが何に依存しているのか」を最初に目で追えるよう、あえて手動定義にしています。  
慣れてきたら`ReflectionContainer`や、`PHP-DI`のような別のDIコンテナも調べてみると理解が深まります。
